...
function workflowSteps(spec: WorkflowAgentSpec): string {
  if (!spec.workflowSteps?.length) return '';
  const steps = spec.workflowSteps.map((step) => {
    const command = step.command ? `\n\n\`\`\`bash\n${step.command}\n\`\`\`` : '';
    const notes = step.notes?.length ? `\n\n${step.notes.map((note) => `- ${note}`).join('\n')}` : '';
    return `### Step ${step.step} — ${step.title}\n\n${step.description ?? ''}${command}${notes}\n`;
  }).join('\n');
  return `## Workflow steps\n\n${steps}`;
}

function utilityCommands(spec: WorkflowAgentSpec): string {
  if (!spec.utilityCommands?.length) return '';
  return `## Utilities\n\n${spec.utilityCommands.map((utility) => `- \`${utility.command}\` — ${utility.purpose}`).join('\n')}\n`;
}
